...
```
これで、そろった状態のルービックキューブをつくります。
```
get_state_value_address();
```
また、これは登録した実体のアドレスを得る関数です。
```
read_csv_file(&ss, "jikkemBooknonbom.csv");
```
これで、csvファイルから、面の色の状態のデータを取り込み、その状態にあるルービックキューブをつくります。
```
make_solved_state(get_state_value_address());
read_text_file(&ss,"jikken.txt");
```
では、そろった状態のルービックキューブを作った後、テキストファイルから、ルービックキューブに対する操作を読み込んでいます。そして、それらの操作によって崩されたルービックキューブをつくります。

＊csvファイル、テキストファイルの入力方式については、下で説明します。

### ルービックキューブを解くための準備
ルービックキューブに対して、解の探索を開始すると、その結果として手数、操作のリストを得ることができます。
```
int m_c = 0;
...
```
これは、操作のリストとなる実体を定義し、とうろくしています。
変数とリストを登録しておくことで、解の探索終了後データがそこに、入れられます。
```
init_motion();
init_transfer_table();
init_prune_table();
...
ルービックキューブを3D空間上に生成するのに必要な情報は、ルービックキューブ生成時に定義した、 d_state  型の実体と面の色に関する情報です。
```
double color[6][3] = {
		{0.0,1.0,0.0},
		{1.0,1.0,0.0},
...
またこの関数を呼ぶと、キーボードによるリアルタイムで様々な処理が可能になります。
