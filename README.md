...
* ビルドして実行。
* シャッフルされたルービックキューブが表示されているのを確認してください。
* [x]キーを押ししばらくすると、標準出力に解法が出力されます。
* 解法が出力された後に[z]キーを押すとルービックキューブが元に戻ります。
### テキストファイルをいじる
先ほどのデモンストレーションで取り込まれた操作は、それほど難しいものではありませんでした。
なので少しファイルを書き換えてみましょう。
```
...
```
read_csv_file(get_state_value_add標準出力に), "ファイル名");
```
...
またこの関数を呼ぶと、キーボード入力をすることで、リアルタイムで様々な操作が可能になります。
